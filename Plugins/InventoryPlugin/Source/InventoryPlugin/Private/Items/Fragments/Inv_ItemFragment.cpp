...
void FInv_HealthPotionFragment::OnConsume(APlayerController* PlayerCont, int32 Count)
{
	...
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, FString::Printf(TEXT("Health Potion consumed! Healing by: %f"), GetValue() * Count));
}
...
void FInv_ManaPotionFragment::OnConsume(APlayerController* PlayerCont, int32 Count)
{
	...
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Blue, FString::Printf(TEXT("Mana Potion consumed! Mana replenished by: %f"), GetValue() * Count));
}
...
void FInv_ConsumableFragment::OnConsume(APlayerController* PC, int32 Count)
{
	for(auto& Modifier : ConsumeModifiers)
	{
		auto& ModRef = Modifier.GetMutable();
		ModRef.OnConsume(PC, Count);
	}
}
